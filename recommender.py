...
from implicit.als import AlternatingLeastSquares # 알고리즘(머신 러닝) 사용을 위해 import
import pickle # 모델 저장을 위해 import, 데이터 저장
import sys
import json
...
def model_train():
  ...
  # iterations : 반복 횟수. 값이 클수록 정확도는 높아지지만, 시간이 오래 걸림. (ex) 50번 확인 횟수)
  ...
  pickle.dump(als_model, open(saved_model_fname, 'wb')) # wb : write binary (바이너리 형태로 문자로 변경)
  return als_model

# 모델을 학습 시켜 model/finalized_model.sav 생성
...
# 사용자가 특정 아이템(여기서는 영화)에 대해 평점을 입력하면, 그 평점이 input_rating_dict로 전달. (데이터를 json 전달)
...
# 이 과정에서 loaded_model.recommend() 함수가 사용되며, N개의 추천 영화를 반환. => count 변환
...
def calculate_user_based(user_items, items):
  ...
  # userid가 0인 경우, 초기화 시켜 사용자의 정보가 새롭게 주어졌을 때, 그 사용자의 특성을 다시 계산
  ...
  # recalculate_user=True : 사용자의 정보가 새롭게 주어졌을 때, 그 사용자의 특성을 다시 계산할지를 결정 => userid=0
...
if __name__ == "__main__":
  ...
  # model_train() # 데이터 학습하여 모델 생성하는 함수. 처음 한번 실행 후 주석 처리 이후 데이터가 추가됐을 때 주기적으로 실행